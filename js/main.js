...
function initBootstrapPopup() {
	jQuery('#collapseExample3, #collapseExample').on('shown.bs.collapse', function () {
		jcf.refreshAll();
	});
}
...
function initAddClasses() {
	...
	jQuery('a.button').clickClass({
		pressedClass: 'pressed'
	});
	jQuery('.notification-drop ul li').clickClass({
		pressedClass: 'pressed',
		classAdd: 'active'
	});
	jQuery('.dropdown-menu ul li a').clickClass({
		pressedClass: 'pressed'
	});
	jQuery('.client-drop ul li a').clickClass({
		pressedClass: 'pressed'
	});
}
...
function initCustomHover() {
	lib.each(lib.queryElementsBySelector('.client-info'), function(index, item) {
		new TouchHover({
			element: item
		});
	});
	lib.each(lib.queryElementsBySelector('.buttons-nav'), function(index, item) {
		new TouchHover({
			element: item
		});
	});
	lib.each(lib.queryElementsBySelector('.info-col h2'), function(index, item) {
		new TouchHover({
			element: item
		});
	});
	lib.each(lib.queryElementsBySelector('.banner-frame .alignleft'), function(index, item) {
		new TouchHover({
			element: item
		});
	});
